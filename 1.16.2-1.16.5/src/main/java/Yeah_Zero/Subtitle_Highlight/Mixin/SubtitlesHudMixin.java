...
public class SubtitlesHudMixin {
    ...
    private Object 获取字幕条目(Iterator 实例) {
        SubtitlesHud.SubtitleEntry 字幕条目 = (SubtitlesHud.SubtitleEntry) 实例.next();
    ...
    private long 字幕时间重定向(SubtitlesHud.SubtitleEntry 实例) {
        return 实例.getTime() - 3000 + Manager.配置项.最长持续时间;
    }
    ...
    private void 字幕显示颜色修改(Args 参数列表) {
        Text 文字 = 参数列表.get(1);
        参数列表.set(1, new LiteralText(文字.getString()).setStyle(文字.getStyle().withColor((TextColor) null)));
        int 红, 绿, 蓝;
    ...
    @ModifyArgs(method = "render(Lnet/minecraft/client/util/math/MatrixStack;)V", at = @At(value = "INVOKE", target = "Lcom/mojang/blaze3d/systems/RenderSystem;translatef(FFF)V"))
    private void 平移修改(Args 参数列表) {
        float 宽度 = -((float) 参数列表.get(0) - this.client.getWindow().getScaledWidth() + 2);
        float 高度 = -((float) 参数列表.get(1) - this.client.getWindow().getScaledHeight() + 30);
    ...
    @ModifyArgs(method = "render(Lnet/minecraft/client/util/math/MatrixStack;)V", at = @At(value = "INVOKE", target = "Lcom/mojang/blaze3d/systems/RenderSystem;scalef(FFF)V"))
