...
public class Screen implements ModMenuApi {
    ...
    public ConfigScreenFactory<?> getModConfigScreenFactory() {
        return (上级界面) -> {
            ConfigBuilder 构建器 = ConfigBuilder.create().setParentScreen(上级界面).setTitle(Text.translatable("subtitle-highlight.configure.title")).setDoesConfirmSave(false).setSavingRunnable(Configuration::保存).setTransparentBackground(true).setDefaultBackgroundTexture(new Identifier("minecraft", "textures/block/white_concrete.png"));
            ...
            构建器.getOrCreateCategory(Text.translatable("subtitle-highlight.configure.category.general"))
                    ...
                    .addEntry(构建器.entryBuilder().startSubCategory(Text.translatable("subtitle-highlight.configure.category.general.sub_category.color")).setExpanded(true).setTooltip(Text.translatable("subtitle-highlight.configure.category.general.sub_category.color.tooltip")).build())
